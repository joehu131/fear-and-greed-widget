...
const scriptContent = `// Fear & Greed Index Widget for Scriptable
// Data Source: CNN Business Fear & Greed Index

const API_URL = "https://production.dataviz.cnn.io/index/fearandgreed/graphdata";
const CACHE_FILE = "fgi_widget_cache.json";

function log(message, ...args) {
  console.log(\`[Fear & Greed Widget] \${message}\`, ...args);
}

function logError(message, ...args) {
  console.error(\`[Fear & Greed Widget] ERROR: \${message}\`, ...args);
}
...
function getCacheManager() {
  try {
    ...
    logError("FileManager not available:", e);
    return null;
  }
}
...
const scriptableData = {
  ...
  name: "Fear & Greed Widget",
  script: scriptContent,
  share_sheet_inputs: []
};

const targetPath = path.resolve(__dirname, '../Fear & Greed Widget.scriptable');
fs.writeFileSync(targetPath, JSON.stringify(scriptableData, null, 2));
console.log('Successfully wrote Fear & Greed Widget.scriptable');
